...
export default function registerChatHandlers(io, socket) {
    ...
    const validateOrderAccess = async (orderId) => {
        try {
            if (!orderId)
                throw new Error('orderId is undefined in validateOrderAccess');
    ...
    socket.on('join_order', async (payload, callback) => {
        try {
            ...
            if (Array.isArray(payload))
                data = payload[0]; // Postman sometimes sends as array
            const { orderId } = data;
            if (!orderId)
                throw new Error('DEBUG: orderId is required in payload');
            ...
            if (typeof callback === 'function')
                callback({ success: true, roomSize: clients.length });
    ...
    socket.on('send_message', async (payload, callback) => {
        try {
            ...
            if (!orderId)
                throw new Error('DEBUG: orderId is required');
