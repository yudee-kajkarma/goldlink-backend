...
export const reassignOrder = async (req, res) => {
    try {
        ...
        await order.save();
        void sendNotification(String(karigarId), 'Order reassigned to you', `You have been assigned order ${order.orderCode}`, { orderId: order._id.toString(), type: 'ORDER_REASSIGNED' });
...
const TERMINAL_STATUSES = ['COMPLETED', 'RECEIVED'];
...
export const getOrderAnalytics = async (req, res) => {
    try {
        ...
        const [totalGenerated, totalCompleted, avgTurnaroundAgg, byKarigar, byStaff, byJewelleryType, monthlyTrend, overdueOrders,] = await Promise.all([
        ...
        const avgTurnaroundDays = avgTurnaroundMs != null && !Number.isNaN(avgTurnaroundMs)
    ...
    try {
        ...
        if (format === 'csv') {
            ...
            for (const o of orders) {
                ...
                const balance = total != null && !Number.isNaN(Number(total)) ? Math.max(0, Number(total) - paid) : '';
                const row = [
                    o.orderCode,
                    o.status,
                    o.jewelleryType,
        ...
        for (const o of orders) {
            ...
            const paid = paidTotal(o);
            const total = o.totalAmount;
            const balance = total != null && !Number.isNaN(Number(total)) ? Math.max(0, Number(total) - paid) : 'n/a';
            doc.text(`${o.orderCode} | ${o.status} | ${o.jewelleryType} | staff: ${created?.name ?? '-'} | karigar: ${assigned?.name ?? '-'} | total: ${total ?? '-'} | paid: ${paid} | due: ${balance}`);
            doc.moveDown(0.25);
        }
        doc.end();
    }
