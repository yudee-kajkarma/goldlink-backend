...
const generateOrderCode = async () => {
    ...
    const counter = await Counter.findOneAndUpdate({ key: counterKey }, { $inc: { seq: 1 } }, { upsert: true, new: true, setDefaultsOnInsert: true });
    // #region agent log
...
export const createOrder = async (req, res) => {
    try {
        const { assignedTo, jewelleryType, metalType, weight, designNotes, purity, expectedDeliveryDate, priority, customerRef, totalAmount, } = req.body;
        ...
        const uploadPromises = files.map(file => s3Service.uploadFile(file.buffer, file.mimetype, 'orders', order._id.toString()));
        const keys = await Promise.all(uploadPromises);
        order.images = keys.map(key => ({ url: key, type: 'INITIAL' }));
        await order.save();
        void sendNotification(String(order.assignedTo), 'New order assigned', `You have been assigned order ${order.orderCode}`, { orderId: order._id.toString(), type: 'ORDER_ASSIGNED' });
...
export const updateOrder = async (req, res) => {
    try {
        ...
        // #endregion
        const allowedFields = ['weight', 'designNotes', 'purity', 'priority', 'expectedDeliveryDate', 'customerRef', 'totalAmount'];
...
// Material endpoints
export const addIssuedMaterial = async (req, res) => {
    try {
        if (!req.body) {
            return res.status(400).json({ success: false, message: 'Request body is missing' });
        }
...
export const updateReturnedMaterial = async (req, res) => {
    try {
