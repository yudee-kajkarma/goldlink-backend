...
router.post('/upload/order/:orderId', protect, uploadMiddleware.array('images', 5), validateMediaSize, uploadOrderImages);
// Upload chat media (single)
router.post('/upload/chat/:orderId', protect, uploadMiddleware.single('media'), validateMediaSize, uploadChatMedia);
// Get secure pre-signed URL for media access
router.get(/^\/media\/(.+)$/, protect, getSecureMediaUrl);
